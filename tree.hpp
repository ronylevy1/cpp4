...
private:
    ...
    void drawNode(sf::RenderWindow& window, Node<T>* node, float x, float y, float offsetX, float offsetY) {
        ...
        sf::Text text(ss.str(), _font, 14);                            // Create text with loaded font
...
public:
   ...
   ~Tree(){
        dfs<T> first = begin_dfs_scan();
        dfs<T> corrent = first;
        dfs<T> last = end_dfs_scan();
        while (corrent != last){
            corrent == last;
            ++first;
            corrent->get_children().clear();
        }
        _root = nullptr;
    }
};
...
public:
    ...
    dfs<T> begin_pre_order() {
        return dfs<T>(_root);
    }

    dfs<T> end_post_order() {
        return dfs<T>(nullptr);
    }

    dfs<T> begin_post_order() {
        return dfs<T>(_root);
    }
